...
/**
 * [ЧИТАТЬ ПЕРЕД ИСПОЛЬЗОВАНИЕМ — audit remediation]
 * $alertsFired/$alerts отражают ТОЛЬКО то, что сервер вернул синхронно в
 * ответе на этот конкретный вызов /v1/ingest. Если детекция на бэкенде
 * асинхронна (обработка через отдельный пайплайн после приёма события),
 * $alertsFired будет равен 0 для КАЖДОГО вызова независимо от того,
 * сработает ли детектор позже — это не подтверждение "атаки не было",
 * а просто "на момент ответа сервер ничего не вернул синхронно". Точная
 * схема (синхронная/асинхронная детекция на текущем бэкенде /v1/ingest)
 * не проверялась в рамках работы над этим SDK — не воспринимайте
 * $alertsFired как живой индикатор результата детекции без подтверждения
 * от команды backend. Для подтверждения того, что конкретный сценарий
 * детекции реально срабатывает, используйте Verification в панели LoGuard,
 * а не эти поля.
 */
